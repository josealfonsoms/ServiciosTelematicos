...
class myHandler(BaseRequestHandler):
    ...
    def handle(self):
        ...
        while True:
            ...
            #If para imprimir solo en el servidor
            if data.decode().startswith("s=>"):
                msg = "[%s:%s] %s" % (host, port, data.decode()[3:])
                print(msg)  # Solo imprime el mensaje en el servidor
...
myServer = ThreadingTCPServer(("127.0.0.1", 7556), myHandler)
SOCKETS_LIST = []
SOCKETS_LIST.append(myServer)
print("ChatServer started on port %s" % 7556)
